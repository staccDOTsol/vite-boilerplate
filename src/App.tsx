...
const App = () => {
  ...
  const contractAddress = 'UQC28Lo21mVhgY0kXqGJcHXamvQewf4DKJ76WkOotsta_MDC';
      ...
      try {
        const contract = client.provider(Address.parse(contractAddress));
          ...
          try {
            ...
            setKeyPrice(Number(fromNano(keyPriceResult.stack.readBigNumber())) + 0.4);
          } catch (error) {
            setKeyPrice(0.4)
    ...
    try {
      await tonConnectUI.sendTransaction({
        ...
        messages: [
          {
            address: contractAddress,
            amount: toNano('0.4').toString(), // Small amount for gas
            payload: beginCell()
